...
global p_pre %previous pressure
global max_p %Maximum pressure 
%%%%%%%%%%Without Pressure%%%%%%%%%%
%add for loop
max_p_list = [0.15 0.1 0.05]; % 15% 10% 5% of initial normal stress %1.5e7-> 30%
for i = 1:length(max_p_list)
    ...
    k_s = 0.9*sigma_initial*(b-a)/L; %k_s=0.8*k_cr; %%%k does not vary with pressure%%% system stiffness
    M = k_s;
    %%%%%%Modified dtheta/dt%%%%%%
...
%     psi0 = 1;
    ...
    F_tau0 = coeff2 * sigma_initial * ( f_o + a * log(abs(v0+coeff3)) + b * log(abs(theta0)));%initial fricition force
    u0 = -F_tau0;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                   coeff1 * (-y(2) - sign(y(3)+coeff3)* ( sigma_initial / ( k_s * L ) ) * ( f_o + a * log(abs(y(3)+coeff3)) + b * log(abs(y(1)))))];
    ...
    res = zeros(round(numofval/1e3),7);
...
%     disp('Pre-simulation Done')
    ...
    counter = 1;
    while t_pre < t_max
        %Get pressure at injection point (@t_pre) %nondimensionalized pressure magnitude
        [p_input,Dp_input] = pressure_center(t_pre,t_inject_start,t_inject_end);
        %Get pressure distribution (@t_pre)
        p0(ceil(end/2)) = p_input;
        %Update diffusion equation (@t_pre -> @t_new=t_pre+t_step)
        f1 = @(t,p) A_mat_sparse * p;
        ...
        [t2,y] = ode23s(@(t,y)sbm_sys_test1(y,coeff1,coeff2,coeff3,coeff4,sigma_initial,f_o,a,b,t_step,alpha,Dp_input),[t_pre,t_pre+t_step],[theta0,u0,v0]);
...
%         psi0 = y(end,4);
    ...
    writematrix(t_list,"spatial"+"_"+"pbc"+"_"+string(max_p)+"_"+"time.txt")
    writematrix(p_list,"spatial"+"_"+"pbc"+"_"+string(max_p)+"_"+"pressure.txt")
    writematrix(res,"res"+"_"+"pbc"+"_"+string(max_p)+"_"+"factors.txt")
end
