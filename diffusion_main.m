...
while t_pre < t_max
        ...
        if t_pre > t_inject_start && t_pre < t_inject_end
            source(1) = Q_input * mu / (permeability(0) * dx);
        ...
        if counter == round(t_compare/t_step)
             ...
             %Store the results
             p_list = [p_list ; p_spatialtotal];
             t_list = [t_list ; t_now];
             %Analytical Solution
             real_p = test_analytical(D,Q_input/2,k_mid,mu,t_compare,x);
             %Compute Error
             ...
             %Plot the results
             figure(50)
             real_xcoord = linspace(-200,200,401);
             fd_xcoord = linspace(-200,200,401);
             plot(fd_xcoord, p_spatialtotal,'k-'); hold on
             plot(real_xcoord, real_p,'r-');
             title('Diffusion Equation w Neumann Boundary Condition')
             xlabel('x coordinate')
             ylabel('pressure')
